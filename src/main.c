...
void Checkargument(int  argc, char** argv)
{
        ...
        if(strcmp(argv[1],"simulation")==0)
        {
                ...
                while(arg_num<argc){
                        ...
                        /************generate stream seed**************/
                        else if(strcmp(argv[arg_num],"-sidx")==0)
                        {
                                sim_start_idx = atoi(argv[++arg_num]);
                                if(sim_times <=0)
                                {
                                        fprintf(stderr,"simulatioin start index must be >= 0");
                                        exit(0);
                                }
                                arg_num++;
                        }
...
int main(int argc, char *argv[])
{
	  ...
	  "____________________________________________________________\n");
        ...
        if(TRACE)
        {
                printf("trace analysis start: k:%d PA:%g Timeinval:%d it:%d tbidx: %d Table Size:%d resolution:%d interth1:%d interth2:%d interspan:%d\n",
