...
int main(int argc, char *argv[])
{
	  ...
	  "____________________________________________________________\n");
        ...
        if(TRACE)
        {
                printf("trace analysis start: k:%d PA:%g Timeinval:%d it:%d Table Index: %d Table Size:%d resolution:%d interth1:%d interth2:%d interspan:%d\n",
                K_Value,pingli_alpha,intervalTime,it,TableIndex,Table_Size,resolution,interpolation_threshold,interpolation_threshold2,interpolation_span);
                Trace_processing(argv[2]);
        }
        // if SIMULATION flag is enabled ,do zipf distribution simulation
        else if(SIMULATION)
        {
                printf("sim start: z:%g length:%d range:%d offset:%d %dTables Table Index:%d Table Size:%d resolution:%d interth1:%d interth2:%d interspan:%d\n"
                ,zipf_par,zipf_slen,zipf_range,zipf_offset,it,TableIndex,Table_Size,resolution,interpolation_threshold,interpolation_threshold2,interpolation_span);
